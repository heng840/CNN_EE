...
from data_load import ACE2005Dataset, pad, all_triggers, all_arguments
...
def train(model, iterator, optimizer, criterion):
    ...
    for i, batch in enumerate(iterator):  # 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标，一般用在 for 循环当中
        tokens_x_2d, triggers_y_2d, arguments_2d, seqlens_1d, words_2d, triggers_2d = batch
        optimizer.zero_grad()  # 意思是把梯度置零
        trigger_predict, triggers_y_2d, trigger_hat_2d, argument_hidden, argument_keys = model.module.predict_triggers(
            tokens_x_2d, triggers_y_2d, arguments_2d
        )
        # 来自model.py，预测触发词
        trigger_predict = trigger_predict.view(-1, trigger_predict.shape[-1])
        trigger_loss = criterion(trigger_predict, triggers_y_2d.view(-1))
        # torch.nn.CrossEntropyLoss
        # Input: Shape (C)或(N, C); output  :()或(N)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_epochs", type=int, default=80)  # epoch=50
    ...
    train_iter = data.DataLoader(dataset=train_dataset,  # ACE返回的量用pad方法
                                 ...
                                 shuffle=False,  # 打乱
                                 sampler=sampler,
                                 # 报错：sampler option is mutually exclusive with shuffle，
                                 # 因为shuffle和sampler不能同时为真，如果sampler不为默认的None的时候，不用设置shuffle属性了
    ...
    "保存模型以继续训练"
    start_epoch = 1
    log_dir = "best_model_CNN.pt"
    log_dir_latest = "latest_model_CNN.pt"
